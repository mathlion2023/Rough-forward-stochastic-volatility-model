function impv = Classical_Impv(x,id,TTM,futures, options,para_yield)

% Implied vol for the classical case
% Copyright
% Mesias Alfeus 2023
    ...
    if(l(4)<length(TTM{id,1}))
        if(l(1)>0)
            for i=1:4
               ...
               call1 = Forward_Analytics(x,Tt(i,1),TF,futures{id,1}(l(i)),options{id,l(i)}(lo(i),2),Nn,abs(ra));
                ...
                call2 = Forward_Analytics(x,Tt(i,2),TF,futures{id,1}(l(i)+1),options{id,l(i)+1}(lo(i),2),Nn,abs(ra));
        ...
        for i=1:4
            if(i==4)
                ...
                call1 = Forward_Analytics(x,Tt(i,1),1,futures{id,1}(l(i)),options{id,l(i)}(lo(i),2),Nn,abs(ra));
                ...
                call2 = Forward_Analytics(x,Tt(i,2),1,futures{id,1}(l(i)+1),options{id,l(i)+1}(lo(i),2),Nn,abs(ra));
                ivm2 = blsimpv(futures{id,1}(l(i)+1),options{id,l(i)+1}(lo(i),2),abs(ra),Tt(i,2),call2);
